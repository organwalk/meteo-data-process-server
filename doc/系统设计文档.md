# meteo-data-process-server 系统设计文档
...
graph LR
    ...
    Gateway --> User[user-client-service]
...
| `user-client-service` | 用户注册、登录、退出、Token 查询 | `UserServiceImpl`、`UserRedisImpl` |
...
sequenceDiagram
    ...
    participant U as user-client-service
...
2. 部署 `user-client-service`，保证网关与业务服务能完成 Token 校验。
